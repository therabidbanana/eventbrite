...
describe Eventbrite::User do
  before(:all) do
    Eventbrite.setup("key")
  end
  
  it "should load from API on initialization" do
    Eventbrite::API.should_receive(:do_post).with("/user_get", anything).and_return(fake_response("/user_get"))
    u = Eventbrite::User.new("fake_key")
  ...
  it "should not call API if no_load set in initialization" do
    Eventbrite::API.should_not_receive(:do_post)
    Eventbrite::User.new("fake_key", true)
  end
  ...
  it "should return a user auth hash" do
    u = Eventbrite::User.new("fake_key", true)
    u2 = Eventbrite::User.new(["fake_email", "fake_pass"], true)
  ...
  it "should retrieve venues" do
    u = Eventbrite::User.new("fake_key", true)
    Eventbrite::API.should_receive(:do_post).with("/user_list_venues", anything).and_return(fake_response("/user_list_venues"))
    v = u.venues
    v.should be_an Eventbrite::VenueCollection
    v.should_not be_empty
  end
  
  it "should cache venues" do
    u = Eventbrite::User.new("fake_key", true)
    Eventbrite::API.should_receive(:do_post).once.with("/user_list_venues", anything).and_return(fake_response("/user_list_venues"))
    u.venues
    u.venues
    Eventbrite::API.should_receive(:do_post).once.with("/user_list_venues", anything).and_return(fake_response("/user_list_venues"))
    u.dirty_venues!
  ...
  it "should retrieve organizers" do
    u = Eventbrite::User.new("fake_key", true)
    Eventbrite::API.should_receive(:do_post).with("/user_list_organizers", anything).and_return(fake_response("/user_list_organizers"))
    o = u.organizers
    o.should be_an Eventbrite::OrganizerCollection
    o.should_not be_empty
  end
  
  it "should cache organizers" do
    u = Eventbrite::User.new("fake_key", true)
    Eventbrite::API.should_receive(:do_post).once.with("/user_list_organizers", anything).and_return(fake_response("/user_list_organizers"))
    u.organizers
    u.organizers
    Eventbrite::API.should_receive(:do_post).once.with("/user_list_organizers", anything).and_return(fake_response("/user_list_organizers"))
    u.dirty_organizers!
  ...
  it "should retrieve events" do
    u = Eventbrite::User.new("fake_key", true)
    Eventbrite::API.should_receive(:do_post).with("/user_list_events", anything).and_return(fake_response("/user_list_events"))
    e = u.events
    e.should be_an Eventbrite::EventCollection
    e.should_not be_empty
  end
  
  it "should cache events" do
    u = Eventbrite::User.new("fake_key", true)
    Eventbrite::API.should_receive(:do_post).once.with("/user_list_events", anything).and_return(fake_response("/user_list_events"))
    u.events
    u.events
    Eventbrite::API.should_receive(:do_post).once.with("/user_list_events", anything).and_return(fake_response("/user_list_events"))
    u.dirty_events!
